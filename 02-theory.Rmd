...
```{=html}
<!--
```{r lossGradients, echo=FALSE}
library(kableExtra)
tableLosses <- data.frame(
  Loss = c(
    "$L_2$: $(y_i - \\phi(x_i))^2$",
    "$L_1$: $|y_i - \\phi(x_i)|$",
    "Huber loss"
  ...
  Gradient = c(
    "$2(y_i - \\phi(x_i))$",
    "$sign(y_i - \\phi(x_i))$",
    "$y_i - \\phi(x_i)$ for $|y_i - \\phi(x_i)| \\leq \\delta$ and $\\delta sign(y_i - \\phi(x_i))$ otherwise, with $\\delta$ quantile of $|y_i - \\phi(x_i)|$"
  )
)

kable(tableLosses, booktabs = TRUE,
      caption = "Gradients of the discussed losses [@elements]")
```
-->
```
+--------------------------------+-----------------------------------------------------------------------------------------+
