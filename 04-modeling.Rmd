...
for 2.
set engine specific arguments in the set_engine() function
use tune() for parameters that should be tuned (optional tune("id_name"))
```{r modelSpec}
# Models:
lm_model <- linear_reg() %>% 
  set_engine("lm")
...
space filling: grid_latin_hypercube(size = 15, original = FALSE)
then tune_grid() function instead of fit_resamples
