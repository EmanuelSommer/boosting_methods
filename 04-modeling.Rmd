...
bout_lm_wflow <- 
  workflow() %>%
  add_model(bout_lm_model) %>%
  add_recipe(burnout_rec_boost)
```

The linear model can directly be fitted:

```{r}
bout_lm_wflow_fitted <- bout_lm_wflow %>%
  fit(burnout_train)

# summary of the fitted basic linear model
summary(pull_workflow_fit(bout_lm_wflow_fitted)$fit)
```
