...
Before one starts with the actual modeling it is crucial to get to know the data and to bring it to the correct format. This process of getting familiar with the data is well known as Exploratory Data Analysis (EDA). To do this many packages are used.\citetext{\citealp{tidyverse}; \citealp{tidymodels}; \citealp{viridis}; \citealp{ggtext}; \citealp[ ]{viridisLite}; \citealp{patchwork}; \citealp{visdat}; \citealp[\citet{latexplots}, \citet{ggally}]{lubridate}} The most important ones will be loaded below.
...
\DocumentationTok{\#\#\# recipe for xgboost (nominal variables must be dummy variables)}
\CommentTok{\# HERE the TRANSFORMED target}
...
\CommentTok{\# XGBoost workflow with the untransformed data}
...
\CommentTok{\# XGBoost workflow with the transformed data (empirical logit)}
...
\CommentTok{\# with the transformed variable}
...
This shows that the model with just stumps converges much faster or may even never reach the low level of when using deeper trees. One reason for this could be that these stumps do not account for interactions and of course the same number of deep trees encode much more information than the same number of stumps.
...
One can observe the exact same ordering. The only difference seems to be a higher influence of the designation variable for the transformed target. This is not surprising as the ressource allocation and designation variable are highly correlated.
...
The performance for the missing values is indeed not that precise but still quite good. There is at least no huge outlier detectable here.

Now the really interesting part is which model performed the best on the test data set. The results can be viewed below in tabular and graphical form.
...
Now fix the number of trees hyperparameter.
...
\includegraphics{boosting_methods_files/figure-latex/unnamed-chunk-105-1.pdf}

Once without the intercept only model.

\includegraphics{boosting_methods_files/figure-latex/unnamed-chunk-106-1.pdf}

So overall the XGBoost model again delivered the best performance on the test data set. The baseline linear model that takes all features without interactions into account also goes a long way and the performance w.r.t. the RMSE metric is even comparable to the Random forest model. All in all one of the major strengths of gradient boosting is displayed here, namely its predictive power. Whether the difference in performance is worth considering such a model is then up to the use case.

In the next and final chapter the most important pros and cons of gradient boosting are discussed.

\hypertarget{conclusion}{%
\chapter{Conclusion}\label{conclusion}}

Now after having a solid theoretical understanding of gradient boosting methods for regression and the implementation XGBoost as well as seeing the application of such models to two data sets one can wrap up this project by discussing the major pros and cons of this approach.
